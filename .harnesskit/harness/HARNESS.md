...
Este diretorio define a camada de Harness Engineering do Harnesskit.
...
> para qualquer nova feature do Harnesskit. Nenhuma feature nao trivial deve ir
...
Use `bash .harnesskit/scripts/harness-doctor.sh` para checar prontidao de
...
Use `.harnesskit/harness/baselines/current.yaml` para documentar avisos conhecidos
...
Use `bash .harnesskit/scripts/harness-evaluate.sh` para executar os sensores de
...
bash .harnesskit/scripts/harness-evaluate.sh --contract .harnesskit/harness/contracts/active/HC-YYYYMMDD-slug.md
bash .harnesskit/scripts/harness-evaluate.sh --include-recommended
```
