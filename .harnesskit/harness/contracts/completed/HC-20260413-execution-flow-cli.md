...
| Evaluator | amy |
| Risk | medium |
| Source Spec | User request: make Harnesskit easier to execute, install with npx, add slash commands, and show next steps |
...
Harnesskit without getting lost in greenfield, brownfield, or feature work.

## Scope

### In Scope

- Add an npm package manifest with a `bin` entry for `harnesskit`.
- Add a Node CLI with `init`, `status`, `next`, `doctor`, `validate`, and
  `contract` commands.
- Make `init` create a clean `.harnesskit` scaffold for a target project instead
  of copying old development artifacts.
- Add user-facing track aliases: `new-product`, `existing-system`, and
  `feature-work`.
- Add slash command prompt files for common Harnesskit actions.
...
- The initial CLI only needs deterministic process intelligence from
  `harnesskit.yaml`, `state.json`, active contracts, and harness doctor output.

## Expected Files or Areas

- `package.json` - npm metadata and bin entry.
- `bin/harnesskit.mjs` - CLI entrypoint.
- `.claude/commands/harnesskit-status.md` - slash command.
- `.claude/commands/harnesskit-next.md` - slash command.
- `.claude/commands/harnesskit-doctor.md` - slash command.
- `.claude/commands/harnesskit-contract.md` - slash command.
- `.harnesskit/core/engine/flow-guide.md` - flow guide and naming policy.
- `.harnesskit/scripts/validate.sh` - required file checks.
- `.harnesskit/core/agents/sheldon.agent.yaml` - user-facing naming update.
...
**Observable truth:** `node bin/harnesskit.mjs status` prints current process
...
**Observable truth:** `node bin/harnesskit.mjs next` explains the next practical
step based on current state and active contracts.

**Verification:**
- `node bin/harnesskit.mjs next`
...
**Observable truth:** `package.json` exposes `harnesskit` under `bin` and the CLI
has an executable shebang.

**Verification:**
- Static review of `package.json` and `bin/harnesskit.mjs`.
...
**Verification:**
- Static review of `.harnesskit/core/engine/flow-guide.md`.
...
**Observable truth:** `bash .harnesskit/scripts/validate.sh` checks the CLI,
...
- **harness-evaluate-contract**: `bash .harnesskit/scripts/harness-evaluate.sh --contract .harnesskit/harness/contracts/completed/HC-20260413-execution-flow-cli.md`

### Recommended

- **cli-init-help**: `node bin/harnesskit.mjs init --help`
...
Evaluation artifact:
`.harnesskit/harness/evaluations/EV-20260413-execution-flow-cli.md`

Amy review artifact:
`.harnesskit/artifacts/review/execution-flow-cli-review.md`
