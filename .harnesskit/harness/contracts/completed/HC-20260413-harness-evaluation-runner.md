...
Implement the first executable Harness evaluation runner for Harnesskit. The
runner must read an active Harness contract, execute or document applicable
sensors from `.harnesskit/harness/sensors.yaml`, and write a durable evaluation
artifact under `.harnesskit/harness/evaluations/`.
...
- The first runner can focus on Harnesskit structural sensors.
...
- `.harnesskit/scripts/harness_evaluate.py` - main evaluation runner.
- `.harnesskit/scripts/harness-evaluate.sh` - executable wrapper.
- `.harnesskit/harness/HARNESS.md` - usage documentation.
- `.harnesskit/harness/sensors.yaml` - optional runner sensor registration.
- `.harnesskit/scripts/validate.sh` - required file registration.
- `.harnesskit/harness/evaluations/EV-20260413-harness-evaluation-runner.md` - generated evaluation evidence.

## Must-Haves

### MH-01 - Runner script exists and is executable through wrapper

**Observable truth:** Running `bash .harnesskit/scripts/harness-evaluate.sh --help` exits successfully and prints usage.

**Verification:**
- `bash .harnesskit/scripts/harness-evaluate.sh --help`
...
**Observable truth:** Running the wrapper with this contract writes a Markdown evaluation under `.harnesskit/harness/evaluations/`.

**Verification:**
- `bash .harnesskit/scripts/harness-evaluate.sh --contract .harnesskit/harness/contracts/active/HC-20260413-harness-evaluation-runner.md`
- `test -f .harnesskit/harness/evaluations/EV-20260413-harness-evaluation-runner.md`
...
**Observable truth:** The evaluation artifact lists `harnesskit-validate` and `harness-doctor` with command, exit status, and blocking status.

**Verification:**
- `rg "harnesskit-validate|harness-doctor|Exit status|Blocks completion" .harnesskit/harness/evaluations/EV-20260413-harness-evaluation-runner.md`
...
**Verification:**
- Static review of `.harnesskit/scripts/harness_evaluate.py` verdict logic.
...
**Verification:**
- `rg "harness_evaluate.py|harness-evaluate.sh" .harnesskit/scripts/validate.sh`
...
- **harnesskit-validate**: `bash .harnesskit/scripts/validate.sh` - checks YAML/JSON and required Harness files.
- **harness-doctor**: `bash .harnesskit/scripts/harness-doctor.sh` - reports Harness feedforward and feedback readiness.
- **harness-evaluate-help**: `bash .harnesskit/scripts/harness-evaluate.sh --help` - verifies the runner is invokable.
- **harness-evaluate-contract**: `bash .harnesskit/scripts/harness-evaluate.sh --contract .harnesskit/harness/contracts/active/HC-20260413-harness-evaluation-runner.md` - verifies the runner can generate evaluation evidence.
...
`.harnesskit/artifacts/review/harness-evaluation-runner-review.md`. The contract
