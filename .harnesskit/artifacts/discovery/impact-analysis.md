...
Melhoria visual completa do framework Harnesskit para ambiente CLI com suporte a Markdown rendering (Claude Code). O framework atual usa formatação básica — emojis de status, tabelas simples, headers. O objetivo é enriquecer a apresentação visual com progress bars, callouts/admonitions, diagramas Mermaid e formatação mais sofisticada, tanto nas respostas dos agentes quanto nos templates de artefatos.
...
O Harnesskit é um framework YAML/Markdown com 7 agentes e 3 tracks. A formatação visual atual inclui:
