# Plano de Harness Engineering para o Harnesskit

## Objetivo

Evoluir o Harnesskit de um framework artifact-driven/spec-driven para um harness de agentes: um sistema que fornece contexto, estado, contratos, sensores e ciclos de correcao para permitir execucoes longas com menor degradacao de qualidade.
...
No Harnesskit, isso ja existe parcialmente por meio de `state.json`, agentes, workflows, handoffs e artefatos. A lacuna esta em transformar esses artefatos em um loop verificavel com sensores e contratos.
...
- `CLAUDE.md`;
- `.harnesskit/core/constitution.md`;
- specs;
...
O Harnesskit hoje e mais forte em feedforward. O MVP de harness deve focar em feedback mecanico e no loop implementador -> avaliador -> corretor.
...
Para o Harnesskit, a direcao e:

- manter `CLAUDE.md` curto;
- usar `.harnesskit/core/` como regras centrais;
- usar `.harnesskit/artifacts/` como memoria de execucao;
...
No Harnesskit, a regra deve ser adaptativa:
...
No Harnesskit, isso vira uma rotina de manutencao:
...
Resultado esperado: o Harnesskit passa a registrar contratos e avaliacoes sem ainda automatizar retries.
...
O Harnesskit pode se diferenciar por:
...
O harness do Harnesskit deve adicionar:
...
Construir a ferramenta como uma evolucao do Harnesskit, nao como um CLI separado no primeiro momento.

Primeiro entregar templates e workflow de contrato/avaliacao. Depois adicionar automacao de sensores e retries. Isso reduz risco porque o Harnesskit ja tem os conceitos centrais: agentes, estado, fases, handoffs e artefatos.
...
- `.harnesskit/harness/feedforward.yaml` para declarar contexto minimo verificavel
- `.harnesskit/scripts/harness_doctor.py` para checar feedforward e feedback
- `.harnesskit/scripts/harness-doctor.sh` como wrapper executavel
- `harness-doctor` adicionado como sensor obrigatorio em `sensors.yaml`
- `harnesskit-validate` tornado sensor obrigatorio de implementacao
...
- nao ha `package.json`, `pyproject.toml` ou `pytest.ini`, entao o feedback de codigo esta limitado aos sensores estruturais do Harnesskit.
...
- `.harnesskit/core/agents/leslie.agent.yaml` como agente dono da criacao de contratos;
- `.harnesskit/core/workflows/harness-contract/` como fluxo guiado para transformar uma unidade de trabalho em contrato verificavel;
...
- `.harnesskit/scripts/harness_evaluate.py` como runner mecanico de avaliacao;
- `.harnesskit/scripts/harness-evaluate.sh` como wrapper executavel;
- geracao de avaliacao Markdown em `.harnesskit/harness/evaluations/`;
...
- documentacao do runner em `.harnesskit/harness/HARNESS.md`;
- `validate.sh` atualizado para exigir runner e wrapper;
- grupo `harness_evaluation` em `.harnesskit/harness/sensors.yaml`.
...
- `.harnesskit/harness/baselines/current.yaml` para registrar findings conhecidos;
- `.harnesskit/harness/baselines/README.md` com politica de baseline;
