# Harnesskit Workflow Engine

This document defines how any AI agent must process and execute an Harnesskit workflow. Follow these instructions exactly.
...
```
1. Read .harnesskit/harness/HARNESS.md
2. Read .harnesskit/harness/sensors.yaml
3. Delegate contract creation to Leslie via .harnesskit/core/workflows/harness-contract/
...
3. Resolve the relative paths to absolute paths within `.harnesskit/`
...
6. If the step description asks for a harness contract, prefer delegating to
   Leslie by loading `.harnesskit/core/agents/leslie.agent.yaml` and executing
   `.harnesskit/core/workflows/harness-contract/workflow.yaml`. The final
   contract is saved in `.harnesskit/harness/contracts/active/`.
7. If the step description asks for harness sensor evidence, read
   `.harnesskit/harness/sensors.yaml`, run or document applicable sensors, and
...
1. Resolve the output path: `output.path` relative to the `.harnesskit/` root
...
1. For each agent in the `handoff_to` array:
   - Add to the `handoffs` array in `state.json`:
     ...
     ```
2. Create a descriptive file in `.harnesskit/handoffs/` with handoff details
