...
1. **Built-in**: `.harnesskit/specs/{name}/spec.yaml`
2. **Local**: `.harnesskit/custom-specs/{name}/spec.yaml`
3. **Installed**: `node_modules/@harnesskit-specs/{name}/spec.yaml` (future)

## Loading Process

1. Read `harnesskit.yaml` to determine which spec is active (`project.spec`)
...
1. Create directory: `.harnesskit/custom-specs/my-spec/`
...
4. Validate: `harnesskit validate-spec my-spec`
5. Activate: set `project.spec` in `harnesskit.yaml`
...
| `VERSION_MISMATCH` | Harnesskit version too old for spec | Update Harnesskit to meet `harnesskit_min_version` |
