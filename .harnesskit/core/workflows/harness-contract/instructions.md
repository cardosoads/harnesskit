...
> 📋 **Important:** While this document is in English, respond to the user in the language configured in `harnesskit.yaml > user_preferences.response_language`.
...
Create a self-contained harness contract in `.harnesskit/harness/contracts/active/` that Howard can build against and Amy can evaluate against.
...
1. Read `.harnesskit/harnesskit.yaml` for response language and active team.
2. Read `.harnesskit/state.json` for current phase and completed artifacts.
3. Read `.harnesskit/harness/HARNESS.md`.
4. Read `.harnesskit/harness/feedforward.yaml`.
5. Read `.harnesskit/harness/sensors.yaml`.
6. Read `.harnesskit/harness/templates/contract-template.md`.
...
- "Running `bash .harnesskit/scripts/harness-doctor.sh` reports 0 errors."
...
Use `.harnesskit/harness/sensors.yaml`.
...
```text
.harnesskit/harness/contracts/active/{{contract_id}}.md
```
