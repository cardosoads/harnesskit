#!/usr/bin/env bash
# =============================================================================
# Harnesskit Validation Script
# =============================================================================
# Validates all YAML and JSON files in the .harnesskit directory for syntax
# correctness. Optionally validates agent YAML files against _schema.json
# if a JSON Schema validator (ajv) is available.
#
# Usage: bash .harnesskit/scripts/validate.sh
# =============================================================================

set -euo pipefail

HARNESSKIT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
PROJECT_ROOT="$(cd "$HARNESSKIT_DIR/.." && pwd)"
ERRORS=0
WARNINGS=0
CHECKED=0
...
while IFS= read -r -d '' file; do
  ...
  CHECKED=$((CHECKED + 1))
done < <(find "$HARNESSKIT_DIR" -name '*.json' -not -path '*/artifacts/*' -print0)
...
if [ -n "$YAML_VALIDATOR" ]; then
  while IFS= read -r -d '' file; do
    ...
    CHECKED=$((CHECKED + 1))
  done < <(find "$HARNESSKIT_DIR" -name '*.yaml' -o -name '*.yml' | sort | tr '\n' '\0')
fi
...
# ---------------------------------------------------------------------------
echo ""
AGENT_SCHEMA="$HARNESSKIT_DIR/core/agents/_schema.json"
if [ -f "$AGENT_SCHEMA" ]; then
  AJV_BIN="${HARNESSKIT_AJV_BIN:-}"
  if [ -n "$AJV_BIN" ] && [ ! -x "$AJV_BIN" ]; then
    log_warn "Configured HARNESSKIT_AJV_BIN is not executable: $AJV_BIN"
    AJV_BIN=""
  fi
  ...
  if [ -n "$AJV_BIN" ]; then
    ...
    while IFS= read -r -d '' file; do
      ...
      rm -rf "$json_tmp_dir"
      CHECKED=$((CHECKED + 1))
    done < <(find "$HARNESSKIT_DIR/core/agents" -name '*.agent.yaml' -print0)
  else
...
REQUIRED_ROOT_FILES=(
  ...
  ".claude/commands/harnesskit-next.md"
  ".claude/commands/harnesskit-doctor.md"
