...
def apply_baselines(root: Path, issues: list[Issue]) -> None:
    baseline_path = root / ".harnesskit" / "harness" / "baselines" / "current.yaml"
...
def check_feedforward(root: Path, issues: list[Issue]) -> None:
    harnesskit = root / ".harnesskit"
    feedforward_path = harnesskit / "harness" / "feedforward.yaml"
    ...
    if not required_context:
        required_context = [
            ...
            {"path": ".harnesskit/harnesskit.yaml", "min_words": 80, "severity": "error"},
            {"path": ".harnesskit/state.json", "min_words": 20, "severity": "error"},
            {"path": ".harnesskit/core/constitution.md", "min_words": 120, "severity": "error"},
            {"path": ".harnesskit/core/engine/workflow-engine.md", "min_words": 200, "severity": "error"},
            {"path": ".harnesskit/core/engine/state-machine.md", "min_words": 160, "severity": "error"},
            {"path": ".harnesskit/harness/HARNESS.md", "min_words": 150, "severity": "error"},
            {"path": ".harnesskit/harness/templates/contract-template.md", "min_words": 120, "severity": "error"},
        ]
    ...
    config = load_yaml(harnesskit / "harnesskit.yaml", issues, "feedforward", "invalid_config") if (harnesskit / "harnesskit.yaml").exists() else {}
    state = load_json(harnesskit / "state.json", issues, "feedforward", "invalid_state") if (harnesskit / "state.json").exists() else {}

    project_type = state.get("project_type") or config.get("project", {}).get("project_type")
    if not project_type:
        issues.append(Issue("error", "feedforward", "missing_project_type", "No project_type found in state.json or harnesskit.yaml"))

    active_specs = config.get("specs", {}).get("active", [])
    if not active_specs:
        issues.append(Issue("warning", "feedforward", "missing_active_specs", "No active specs declared in harnesskit.yaml", "harnesskit.yaml"))
    for spec in active_specs:
        spec_path = harnesskit / "specs" / str(spec) / "spec.yaml"
    ...
    if active_team:
        team_path = harnesskit / "teams" / f"{active_team}.yaml"
        if not team_path.exists():
            add_missing_file(issues, root, "feedforward", team_path)
    else:
        issues.append(Issue("warning", "feedforward", "missing_active_team", "No active team declared in harnesskit.yaml", "harnesskit.yaml"))
        ...
        for phase, pdata in state_phases.items():
            for step in pdata.get("steps", []) if isinstance(pdata, dict) else []:
                ...
                if artifact and status in {"completed", "in_progress"}:
                    artifact_path = harnesskit / str(artifact)
    ...
    if not workflow_requirements:
        workflow_requirements = [
            {"path": ".harnesskit/core/workflows/4-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "record-harness-sensors"]},
            {"path": ".harnesskit/core/workflows/bf-3-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "record-harness-sensors"]},
            {"path": ".harnesskit/core/workflows/nf-4-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "run-harness-sensors"]},
        ]
    ...
    for contract in (harnesskit / "harness" / "contracts" / "active").glob("*.md"):
    ...
    if feedforward.get("artifact_quality", {}).get("implementation_report_requires_harness", True):
        implementation_reports = list((harnesskit / "artifacts" / "implementation").glob("*.md"))
...
def check_feedback(root: Path, issues: list[Issue]) -> None:
    harnesskit = root / ".harnesskit"
    sensors_path = harnesskit / "harness" / "sensors.yaml"
    ...
    if not ({"harnesskit-validate", "harnesskit-validate"} & required_ids):
        issues.append(Issue("warning", "feedback", "missing_harnesskit_validate_sensor", "harnesskit-validate is not required for implementation", rel(root, sensors_path)))
    ...
    if not discovered_checks:
        issues.append(Issue("info", "feedback", "no_code_test_runner_discovered", "No package.json/pyproject/pytest test runner discovered; feedback is limited to Harnesskit structural sensors"))

    evaluations = [p for p in (harnesskit / "harness" / "evaluations").glob("*.md") if p.name != ".gitkeep"]
    active_contracts = [p for p in (harnesskit / "harness" / "contracts" / "active").glob("*.md")]
    completed_contracts = [p for p in (harnesskit / "harness" / "contracts" / "completed").glob("*.md")]
    if active_contracts and not evaluations:
        issues.append(Issue("warning", "feedback", "active_contract_without_evaluation", "Active harness contracts exist but no evaluation artifact has been recorded", rel(root, harnesskit / "harness" / "evaluations")))
    if completed_contracts and not evaluations:
        issues.append(Issue("warning", "feedback", "completed_contract_without_evaluation", "Completed harness contracts exist but no evaluation artifact has been recorded", rel(root, harnesskit / "harness" / "evaluations")))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Harnesskit feedforward and feedback readiness.")
    ...
    issues: list[Issue] = []
    if not (root / ".harnesskit").exists():
        issues.append(Issue("error", "feedforward", "missing_harnesskit_dir", "No .harnesskit directory found", str(root)))
    else:
