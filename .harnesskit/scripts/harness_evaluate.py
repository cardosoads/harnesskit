#!/usr/bin/env python3
"""
Harnesskit Harness Evaluation Runner.
...
def main() -> int:
    ...
    parser.add_argument("--sensor-group", default="implementation", help="Sensor group from .harnesskit/harness/sensors.yaml")
    parser.add_argument("--output", help="Evaluation output path. Defaults to .harnesskit/harness/evaluations/<evaluation-id>.md")
    ...
    if not harnesskit.exists():
        raise SystemExit(f"No .harnesskit directory found under {root}")

    if args.contract:
        contract_path = resolve_cli_path(root, args.contract).resolve()
    else:
        try:
            contract_path = newest_contract(harnesskit / "harness" / "contracts" / "active")
    ...
    config = load_yaml(harnesskit / "harnesskit.yaml") if (harnesskit / "harnesskit.yaml").exists() else {}
    ...
    output_path = resolve_cli_path(root, args.output).resolve() if args.output else harnesskit / "harness" / "evaluations" / f"{evaluation_id}.md"
