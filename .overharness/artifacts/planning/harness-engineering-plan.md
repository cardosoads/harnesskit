# Plano de Harness Engineering para o OverHarness

## Objetivo

Evoluir o OverHarness de um framework artifact-driven/spec-driven para um harness de agentes: um sistema que fornece contexto, estado, contratos, sensores e ciclos de correcao para permitir execucoes longas com menor degradacao de qualidade.
...
No OverHarness, isso ja existe parcialmente por meio de `state.json`, agentes, workflows, handoffs e artefatos. A lacuna esta em transformar esses artefatos em um loop verificavel com sensores e contratos.
...
- `CLAUDE.md`;
- `.overharness/core/constitution.md`;
- specs;
...
O OverHarness hoje e mais forte em feedforward. O MVP de harness deve focar em feedback mecanico e no loop implementador -> avaliador -> corretor.
...
Para o OverHarness, a direcao e:

- manter `CLAUDE.md` curto;
- usar `.overharness/core/` como regras centrais;
- usar `.overharness/artifacts/` como memoria de execucao;
...
No OverHarness, a regra deve ser adaptativa:
...
No OverHarness, isso vira uma rotina de manutencao:
...
Resultado esperado: o OverHarness passa a registrar contratos e avaliacoes sem ainda automatizar retries.
...
O OverHarness pode se diferenciar por:
...
O harness do OverHarness deve adicionar:
...
Construir a ferramenta como uma evolucao do OverHarness, nao como um CLI separado no primeiro momento.

Primeiro entregar templates e workflow de contrato/avaliacao. Depois adicionar automacao de sensores e retries. Isso reduz risco porque o OverHarness ja tem os conceitos centrais: agentes, estado, fases, handoffs e artefatos.
...
- `.overharness/harness/feedforward.yaml` para declarar contexto minimo verificavel
- `.overharness/scripts/harness_doctor.py` para checar feedforward e feedback
- `.overharness/scripts/harness-doctor.sh` como wrapper executavel
- `harness-doctor` adicionado como sensor obrigatorio em `sensors.yaml`
- `overharness-validate` tornado sensor obrigatorio de implementacao
...
- nao ha `package.json`, `pyproject.toml` ou `pytest.ini`, entao o feedback de codigo esta limitado aos sensores estruturais do OverHarness.
...
- `.overharness/core/agents/leslie.agent.yaml` como agente dono da criacao de contratos;
- `.overharness/core/workflows/harness-contract/` como fluxo guiado para transformar uma unidade de trabalho em contrato verificavel;
...
- `.overharness/scripts/harness_evaluate.py` como runner mecanico de avaliacao;
- `.overharness/scripts/harness-evaluate.sh` como wrapper executavel;
- geracao de avaliacao Markdown em `.overharness/harness/evaluations/`;
...
- documentacao do runner em `.overharness/harness/HARNESS.md`;
- `validate.sh` atualizado para exigir runner e wrapper;
- grupo `harness_evaluation` em `.overharness/harness/sensors.yaml`.
...
- `.overharness/harness/baselines/current.yaml` para registrar findings conhecidos;
- `.overharness/harness/baselines/README.md` com politica de baseline;
