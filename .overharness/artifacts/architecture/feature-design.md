...
**Princípio arquitetural:** O OverHarness não tem runtime — toda formatação é declarativa. Agentes "leem" as instruções e formatam output de acordo. Portanto, melhorar a formatação = melhorar as instruções de formatação.
...
| Seção em `overharness.yaml` | Junto com config | YAML não é bom para exemplos visuais | Não |
