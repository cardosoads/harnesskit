...
# =============================================================================
# Validates all YAML and JSON files in the .overharness directory for syntax
# correctness. Optionally validates agent YAML files against _schema.json
# if a JSON Schema validator (ajv) is available.
#
# Usage: bash .overharness/scripts/validate.sh
# =============================================================================

set -euo pipefail

OVERHARNESS_DIR="$(cd "$(dirname "$0")/.." && pwd)"
PROJECT_ROOT="$(cd "$OVERHARNESS_DIR/.." && pwd)"
ERRORS=0
WARNINGS=0
CHECKED=0
...
while IFS= read -r -d '' file; do
  ...
  CHECKED=$((CHECKED + 1))
done < <(find "$OVERHARNESS_DIR" -name '*.json' -not -path '*/artifacts/*' -print0)
...
if [ -n "$YAML_VALIDATOR" ]; then
  while IFS= read -r -d '' file; do
    ...
    CHECKED=$((CHECKED + 1))
  done < <(find "$OVERHARNESS_DIR" -name '*.yaml' -o -name '*.yml' | sort | tr '\n' '\0')
fi
...
# ---------------------------------------------------------------------------
echo ""
AGENT_SCHEMA="$OVERHARNESS_DIR/core/agents/_schema.json"
if [ -f "$AGENT_SCHEMA" ]; then
  AJV_BIN="${OVERHARNESS_AJV_BIN:-}"
  if [ -n "$AJV_BIN" ] && [ ! -x "$AJV_BIN" ]; then
    log_warn "Configured OVERHARNESS_AJV_BIN is not executable: $AJV_BIN"
    AJV_BIN=""
  fi
  ...
  if [ -n "$AJV_BIN" ]; then
    ...
    while IFS= read -r -d '' file; do
      ...
      rm -rf "$json_tmp_dir"
      CHECKED=$((CHECKED + 1))
    done < <(find "$OVERHARNESS_DIR/core/agents" -name '*.agent.yaml' -print0)
  else
