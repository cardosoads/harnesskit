...
- Add `.overharness/harness/baselines/current.yaml` with documented baseline entries.
...
- `.overharness/scripts/harness_doctor.py` - baseline matching and output.
- `.overharness/harness/baselines/current.yaml` - current accepted baseline.
- `.overharness/harness/baselines/README.md` - baseline policy.
- `.overharness/harness/HARNESS.md` - user-facing documentation.
- `.overharness/harness/feedforward.yaml` - required context registration.
- `.overharness/scripts/validate.sh` - required file and directory registration.
- `.overharness/harness/evaluations/EV-20260413-harness-baseline-exceptions.md` - generated evaluation evidence.

## Must-Haves

### MH-01 - Baseline file exists and documents current warnings

**Observable truth:** `.overharness/harness/baselines/current.yaml` exists and includes the four current doctor warnings as baseline entries.

**Verification:**
- `test -f .overharness/harness/baselines/current.yaml`
- `rg "phase_missing_in_state|artifact_placeholder|completed_step_without_artifact|implementation_report_missing_harness" .overharness/harness/baselines/current.yaml`
...
**Observable truth:** Running `bash .overharness/scripts/harness-doctor.sh` reports 0 warnings for the four baseline entries and reports them as baseline info.
...
**Observable truth:** `bash .overharness/scripts/harness-doctor.sh --format json` includes baseline metadata for matched issues.

**Verification:**
- `bash .overharness/scripts/harness-doctor.sh --format json`
- `rg "\"baseline\"" .overharness/harness/evaluations/EV-20260413-harness-baseline-exceptions.md`
...
**Verification:**
- `rg "harness/baselines" .overharness/scripts/validate.sh`
...
- **overharness-validate**: `bash .overharness/scripts/validate.sh` - checks required Harness files.
- **harness-doctor**: `bash .overharness/scripts/harness-doctor.sh` - verifies baseline reclassification.
- **harness-evaluate-contract**: `bash .overharness/scripts/harness-evaluate.sh --contract .overharness/harness/contracts/active/HC-20260413-harness-baseline-exceptions.md` - writes evaluation evidence.

### Recommended

- **doctor-json**: `bash .overharness/scripts/harness-doctor.sh --format json` - confirms machine-readable baseline metadata.
...
`.overharness/artifacts/review/harness-baseline-exceptions-review.md`. The
