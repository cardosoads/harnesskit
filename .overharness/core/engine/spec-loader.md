...
1. **Built-in**: `.overharness/specs/{name}/spec.yaml`
2. **Local**: `.overharness/custom-specs/{name}/spec.yaml`
3. **Installed**: `node_modules/@overharness-specs/{name}/spec.yaml` (future)

## Loading Process

1. Read `overharness.yaml` to determine which spec is active (`project.spec`)
...
1. Create directory: `.overharness/custom-specs/my-spec/`
...
5. Activate: set `project.spec` in `overharness.yaml`
...
| `VERSION_MISMATCH` | OverHarness version too old for spec | Update OverHarness to meet `overharness_min_version` |
