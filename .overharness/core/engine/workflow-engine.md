...
```
1. Read .overharness/harness/HARNESS.md
2. Read .overharness/harness/sensors.yaml
3. Delegate contract creation to Leslie via .overharness/core/workflows/harness-contract/
...
3. Resolve the relative paths to absolute paths within `.overharness/`
...
6. If the step description asks for a harness contract, prefer delegating to
   Leslie by loading `.overharness/core/agents/leslie.agent.yaml` and executing
   `.overharness/core/workflows/harness-contract/workflow.yaml`. The final
   contract is saved in `.overharness/harness/contracts/active/`.
7. If the step description asks for harness sensor evidence, read
   `.overharness/harness/sensors.yaml`, run or document applicable sensors, and
...
1. Resolve the output path: `output.path` relative to the `.overharness/` root
...
1. For each agent in the `handoff_to` array:
   - Add to the `handoffs` array in `state.json`:
     ...
     ```
2. Create a descriptive file in `.overharness/handoffs/` with handoff details
