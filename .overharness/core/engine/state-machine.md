...
```
# Read the phase order from overharness.yaml based on the active project_type:
...
phase_list = read overharness.yaml > {project_type}_phases, sorted by order
...
Some phases have `required: false` in overharness.yaml (e.g., `discuss`, `design`). These phases are **optional** — they can be skipped without breaking the flow.
...
1. Add an entry to the `handoffs` array in `state.json`:
   ...
   ```
2. Create a handoff file in `.overharness/handoffs/` with details
