...
async function promptForInit(args, targetRoot) {
  ...
  if (!isInteractive()) {
    if (!name || !track) {
      ...
      console.error("Use: harnesskit init --type feature-work --name <project-name>");
      console.error("Or:  harnesskit init --yes --name <project-name>");
      process.exit(1);
    }
    return { name, track };
  }
  ...
  try {
    console.log("Harnesskit init wizard");
    if (!name) {
...
function findProjectRoot(start = process.cwd()) {
  ...
  while (true) {
    if (existsSync(path.join(current, ".harnesskit"))) return current;
...
function loadState(root) {
  const statePath = path.join(root, ".harnesskit", "state.json");
...
function readProjectType(root, state) {
  ...
  const config = path.join(root, ".harnesskit", "harnesskit.yaml");
...
function activeContracts(root) {
  const activeDir = path.join(root, ".harnesskit", "harness", "contracts", "active");
...
function printStatus(root) {
  ...
  if (!state) {
    ...
    console.log("Run: npx @cardosoads/harnesskit@latest init --type feature-work --name <project-name>");
    return;
  }
...
function printNext(root) {
  ...
  if (contracts.length) {
    ...
    console.log("  Run: npx @cardosoads/harnesskit@latest doctor");
  ...
  if (next?.step) {
    ...
    console.log("  Claude Code slash command: /harnesskit-next");
    return;
  }
  console.log("  Create the next Harness contract for the feature or improvement you want.");
  console.log('  Run: npx @cardosoads/harnesskit@latest contract "describe the work unit"');
...
function printCodexGuide(root) {
  ...
  console.log("Harnesskit for Codex");
  console.log("");
  console.log(`Project: ${projectName(root, state)}`);
  console.log(`Track: ${trackInfo.label} (${trackInfo.alias}; internal: ${track})`);
  ...
  console.log("");
  if (contracts.length) {
    console.log(`Recommended next action: continue ${contracts[0]} and run npx @cardosoads/harnesskit@latest doctor.`);
  } else if (next?.step) {
    console.log(`Recommended next action: ask Codex to route ${next.phase}/${next.step.id} to ${next.step.agent || "the responsible agent"}.`);
  } else {
    console.log('Recommended next action: run npx @cardosoads/harnesskit@latest contract "describe the work unit".');
  }
}
...
function ensureCleanDirs(root) {
  ...
  for (const dir of dirs) writeText(path.join(root, ".harnesskit", dir, ".gitkeep"), "");
}
...
function writeConfig(targetRoot, name, track) {
  const templatePath = path.join(packageRoot, ".harnesskit", "harnesskit.yaml");
  let text = existsSync(templatePath) ? readText(templatePath) : "";
  if (!text) throw new Error("Missing packaged .harnesskit/harnesskit.yaml template.");
  text = text.replace(/name:\s*"[^"]*"/, `name: "${name}"`);
  text = text.replace(/description:\s*"[^"]*"/, 'description: "Harnesskit project scaffold"');
  text = text.replace(/project_type:\s*"[^"]*"/, `project_type: "${track}"`);
  text = text.replace(/active:\s*"team-[^"]*"/, `active: "${TRACKS[track].team}"`);
  writeText(path.join(targetRoot, ".harnesskit", "harnesskit.yaml"), text);
}

function writeEmptyBaselines(targetRoot) {
  writeText(
    path.join(targetRoot, ".harnesskit", "harness", "baselines", "current.yaml"),
...
function writeSlashCommands(targetRoot) {
  const commands = {
    "harnesskit-status.md": "Read `.harnesskit/state.json`, run `npx @cardosoads/harnesskit@latest status`, and explain where the project is in the Harnesskit process. Respond in the configured user language.\n",
    "harnesskit-next.md": "Run or emulate `npx @cardosoads/harnesskit@latest next`, then recommend the next Harnesskit action. If work is non-trivial, route through Sheldon -> Leslie contract -> Howard -> sensors -> Amy.\n",
    "harnesskit-doctor.md": "Run `npx @cardosoads/harnesskit@latest doctor` and summarize feedforward/feedback findings. Do not hide baseline debt.\n",
    "harnesskit-contract.md": "Guide the user through Leslie's Harness contract workflow for the requested work unit. If the work unit is known, run `npx @cardosoads/harnesskit@latest contract \"describe the work unit\"`; otherwise draft the questions Leslie needs before implementation.\n",
  };
...
async function initProject(args) {
  if (hasFlag(args, "--help") || hasFlag(args, "-h")) {
    console.log("Usage: harnesskit init [--type <new-product|existing-system|feature-work>] [--name <name>] [--yes] [--force]");
    return;
  }
  const targetRoot = process.cwd();
  if (existsSync(path.join(targetRoot, ".harnesskit")) && !hasFlag(args, "--force")) {
    console.error("Harnesskit already exists here. Use --force only when you intend to replace the scaffold.");
    process.exit(1);
  }
  ...
  const templateRoot = path.join(packageRoot, ".harnesskit");

  for (const dir of ["core", "schemas", "scripts", "specs", "teams"]) {
    copyDirectory(path.join(templateRoot, dir), path.join(targetRoot, ".harnesskit", dir));
  }
  copyDirectory(path.join(templateRoot, "harness", "templates"), path.join(targetRoot, ".harnesskit", "harness", "templates"));
  copyDirectory(path.join(templateRoot, "harness", "baselines"), path.join(targetRoot, ".harnesskit", "harness", "baselines"));
  for (const file of ["HARNESS.md", "feedforward.yaml", "sensors.yaml"]) {
    cpSync(path.join(templateRoot, "harness", file), path.join(targetRoot, ".harnesskit", "harness", file));
  }
  if (existsSync(path.join(templateRoot, "harness", "contracts", "README.md"))) {
    mkdirSync(path.join(targetRoot, ".harnesskit", "harness", "contracts"), { recursive: true });
    cpSync(path.join(templateRoot, "harness", "contracts", "README.md"), path.join(targetRoot, ".harnesskit", "harness", "contracts", "README.md"));
  }
  writeConfig(targetRoot, name, track);
  writeText(path.join(targetRoot, ".harnesskit", "state.json"), `${JSON.stringify(freshState(name, track), null, 2)}\n`);
...
function createContract(root, args) {
  ...
  if (!workUnit || hasFlag(args, "--help") || hasFlag(args, "-h")) {
    console.log('Usage: harnesskit contract "describe the work unit"');
    return;
  }
  const slug = workUnit.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60) || "work-unit";
  const today = new Date().toISOString().slice(0, 10).replaceAll("-", "");
  const contractId = `HC-${today}-${slug}`;
  const file = path.join(root, ".harnesskit", "harness", "contracts", "active", `${contractId}.md`);
  if (existsSync(file)) {
...
async function main() {
  ...
  if (!root) {
    console.error("No .harnesskit directory found. Run: npx @cardosoads/harnesskit@latest init --type feature-work");
    process.exit(1);
  }
  ...
  if (command === "validate") {
    ...
    const env = existsSync(ajvBin) ? { HARNESSKIT_AJV_BIN: ajvBin } : {};
    return runProjectCommand(root, ["bash", ".harnesskit/scripts/validate.sh"], env);
  }
