...
- Add `.overspec/harness/baselines/current.yaml` with documented baseline entries.
...
- `.overspec/scripts/harness_doctor.py` - baseline matching and output.
- `.overspec/harness/baselines/current.yaml` - current accepted baseline.
- `.overspec/harness/baselines/README.md` - baseline policy.
- `.overspec/harness/HARNESS.md` - user-facing documentation.
- `.overspec/harness/feedforward.yaml` - required context registration.
- `.overspec/scripts/validate.sh` - required file and directory registration.
- `.overspec/harness/evaluations/EV-20260413-harness-baseline-exceptions.md` - generated evaluation evidence.

## Must-Haves

### MH-01 - Baseline file exists and documents current warnings

**Observable truth:** `.overspec/harness/baselines/current.yaml` exists and includes the four current doctor warnings as baseline entries.

**Verification:**
- `test -f .overspec/harness/baselines/current.yaml`
- `rg "phase_missing_in_state|artifact_placeholder|completed_step_without_artifact|implementation_report_missing_harness" .overspec/harness/baselines/current.yaml`
...
**Observable truth:** Running `bash .overspec/scripts/harness-doctor.sh` reports 0 warnings for the four baseline entries and reports them as baseline info.

**Verification:**
- `bash .overspec/scripts/harness-doctor.sh`
...
**Observable truth:** `bash .overspec/scripts/harness-doctor.sh --format json` includes baseline metadata for matched issues.

**Verification:**
- `bash .overspec/scripts/harness-doctor.sh --format json`
- `rg "\"baseline\"" .overspec/harness/evaluations/EV-20260413-harness-baseline-exceptions.md`
...
**Verification:**
- Static review of `.overspec/scripts/harness_doctor.py`.
...
**Verification:**
- `rg "harness/baselines" .overspec/scripts/validate.sh`
...
- **overspec-validate**: `bash .overspec/scripts/validate.sh` - checks required Harness files.
- **harness-doctor**: `bash .overspec/scripts/harness-doctor.sh` - verifies baseline reclassification.
- **harness-evaluate-contract**: `bash .overspec/scripts/harness-evaluate.sh --contract .overspec/harness/contracts/active/HC-20260413-harness-baseline-exceptions.md` - writes evaluation evidence.

### Recommended

- **doctor-json**: `bash .overspec/scripts/harness-doctor.sh --format json` - confirms machine-readable baseline metadata.
...
`.overspec/artifacts/review/harness-baseline-exceptions-review.md`. The
