...
| Evaluator | amy |
| Risk | low |
| Source Spec | User request: install ajv-cli when OverSpec is downloaded from npm |
| Source Design | `package.json`, `.overspec/scripts/validate.sh` |
| Source Review | `.overspec/artifacts/review/bundled-ajv-cli-review.md` |
| Status | completed |

## Work Unit

Bundle `ajv-cli` with the npm package and make OverSpec validation use the
...
- Let `overspec validate` expose the packaged `ajv` binary to `validate.sh`.
- Let `validate.sh` prefer `OVERSPEC_AJV_BIN`, then local project `node_modules`,
  then global `ajv`.
- Keep direct `bash .overspec/scripts/validate.sh` working.
...
**Observable truth:** `overspec validate` sets `OVERSPEC_AJV_BIN` when the
package-local `node_modules/.bin/ajv` exists.

**Verification:**
- Static review of `bin/overspec.mjs`.

---

### MH-03 - validate.sh can use local or global ajv

**Observable truth:** `validate.sh` checks `OVERSPEC_AJV_BIN`,
...
Evaluation artifact:
`.overspec/harness/evaluations/EV-20260413-bundled-ajv-cli.md`

Amy review artifact:
`.overspec/artifacts/review/bundled-ajv-cli-review.md`
