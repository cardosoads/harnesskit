...
| Evaluator | amy |
| Risk | medium |
| Source Spec | `.overspec/harness/contracts/completed/HC-20260413-harness-baseline-exceptions.md` |
...
- `.overspec/scripts/harness_doctor.py` - expiration logic and output metadata.
- `.overspec/harness/baselines/current.yaml` - `expires_at` on each entry.
- `.overspec/harness/baselines/README.md` - expiration policy.
- `.overspec/harness/HARNESS.md` - user-facing rule summary.
- `.overspec/harness/evaluations/EV-20260413-baseline-expiration-policy.md` - evaluation evidence.

## Must-Haves

### MH-01 - Baseline entries have expiration dates

**Observable truth:** Every entry in `.overspec/harness/baselines/current.yaml` includes `expires_at`.

**Verification:**
- `rg "expires_at" .overspec/harness/baselines/current.yaml`
...
**Observable truth:** `bash .overspec/scripts/harness-doctor.sh --format json` includes `baseline_expires_at` for accepted baseline findings.
...
- **overspec-validate**: `bash .overspec/scripts/validate.sh` - checks Harness files.
- **harness-doctor**: `bash .overspec/scripts/harness-doctor.sh` - verifies current baselines are accepted and not expired.
- **harness-evaluate-contract**: `bash .overspec/scripts/harness-evaluate.sh --contract .overspec/harness/contracts/completed/HC-20260413-baseline-expiration-policy.md` - writes evaluation evidence.

### Recommended

- **doctor-json**: `bash .overspec/scripts/harness-doctor.sh --format json` - confirms `baseline_expires_at` metadata.
...
Evaluation artifact:
`.overspec/harness/evaluations/EV-20260413-baseline-expiration-policy.md`

Amy review artifact:
`.overspec/artifacts/review/harness-baseline-expiration-policy-review.md`
