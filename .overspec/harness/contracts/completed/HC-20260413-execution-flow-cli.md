...
| Evaluator | amy |
| Risk | medium |
| Source Spec | User request: make OverSpec easier to execute, install with npx, add slash commands, and show next steps |
| Source Design | `.overspec/harness/HARNESS.md` |
| Source Planning/Analysis | `.overspec/artifacts/analysis/current-harness-readiness-2026-04-13.md` |
| Status | active |

## Work Unit

Add a first executable CLI and slash-command layer so users can install and run
OverSpec without getting lost in greenfield, brownfield, or feature work.

## Scope

### In Scope

- Add an npm package manifest with a `bin` entry for `overspec`.
- Add a Node CLI with `init`, `status`, `next`, `doctor`, `validate`, and
  `contract` commands.
- Make `init` create a clean `.overspec` scaffold for a target project instead
  of copying old development artifacts.
- Add user-facing track aliases: `new-product`, `existing-system`, and
  `feature-work`.
- Add slash command prompt files for common OverSpec actions.
...
- Mutating `.overspec/state.json` for this development project.
...
- The initial CLI only needs deterministic process intelligence from
  `overspec.yaml`, `state.json`, active contracts, and harness doctor output.

## Expected Files or Areas

- `package.json` - npm metadata and bin entry.
- `bin/overspec.mjs` - CLI entrypoint.
- `.claude/commands/overspec-status.md` - slash command.
- `.claude/commands/overspec-next.md` - slash command.
- `.claude/commands/overspec-doctor.md` - slash command.
- `.claude/commands/overspec-contract.md` - slash command.
- `.overspec/core/engine/flow-guide.md` - flow guide and naming policy.
- `.overspec/scripts/validate.sh` - required file checks.
- `.overspec/core/agents/sheldon.agent.yaml` - user-facing naming update.
...
**Observable truth:** `node bin/overspec.mjs status` prints current process
state and exits 0 inside this repo.

**Verification:**
- `node bin/overspec.mjs status`
...
**Observable truth:** `node bin/overspec.mjs next` explains the next practical
step based on current state and active contracts.

**Verification:**
- `node bin/overspec.mjs next`
...
**Observable truth:** `package.json` exposes `overspec` under `bin` and the CLI
has an executable shebang.

**Verification:**
- Static review of `package.json` and `bin/overspec.mjs`.
...
**Verification:**
- Static review of `.overspec/core/engine/flow-guide.md`.
...
**Observable truth:** `bash .overspec/scripts/validate.sh` checks the CLI,
package manifest, flow guide, and slash commands.

**Verification:**
- `bash .overspec/scripts/validate.sh`
...
- **overspec-validate**: `bash .overspec/scripts/validate.sh`
- **harness-doctor**: `bash .overspec/scripts/harness-doctor.sh`
- **harness-selftest**: `bash .overspec/scripts/harness-selftest.sh`
- **overspec-cli-status**: `node bin/overspec.mjs status`
- **overspec-cli-next**: `node bin/overspec.mjs next`
- **harness-evaluate-contract**: `bash .overspec/scripts/harness-evaluate.sh --contract .overspec/harness/contracts/completed/HC-20260413-execution-flow-cli.md`

### Recommended

- **cli-init-help**: `node bin/overspec.mjs init --help`
...
Evaluation artifact:
`.overspec/harness/evaluations/EV-20260413-execution-flow-cli.md`

Amy review artifact:
`.overspec/artifacts/review/execution-flow-cli-review.md`

Approved for completion and moved to
`.overspec/harness/contracts/completed/`.
