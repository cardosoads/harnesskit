...
| Evaluator | amy |
| Risk | medium |
| Source Spec | `.overspec/harness/contracts/completed/HC-20260413-execution-flow-cli.md` |
| Source Design | `.overspec/core/engine/flow-guide.md` |
| Source Planning/Analysis | `.overspec/artifacts/review/execution-flow-cli-review.md` |
| Status | active |

## Work Unit

Make `overspec init` easier for first-time use by adding an interactive wizard
...
- Add interactive prompts to `overspec init` for project name and track.
...
- `bin/overspec.mjs` - init wizard and non-interactive behavior.
- `package.json` - npm scripts/metadata for pack checks.
- `README.md` - user-facing npm usage.
- `.overspec/core/engine/flow-guide.md` - wizard and publishing notes.
- `.overspec/scripts/validate.sh` - README required-file check.
- `.overspec/harness/evaluations/EV-20260413-init-wizard-publish-prep.md` -
  evaluation evidence.
...
**Observable truth:** `node bin/overspec.mjs init --type feature-work --name sample`
...
**Observable truth:** `node bin/overspec.mjs init --yes --name sample` uses a
...
**Observable truth:** `bin/overspec.mjs` uses Node built-ins for prompting and
does not add runtime dependencies.

**Verification:**
- Static review of `bin/overspec.mjs` and `package.json`.
...
**Observable truth:** `README.md` documents `npx overspec init`, track names,
...
Amy review artifact:
`.overspec/artifacts/review/init-wizard-publish-prep-review.md`
