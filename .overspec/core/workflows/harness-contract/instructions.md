...
> 📋 **Important:** While this document is in English, respond to the user in the language configured in `overspec.yaml > user_preferences.response_language`.
...
Create a self-contained harness contract in `.overspec/harness/contracts/active/` that Howard can build against and Amy can evaluate against.
...
1. Read `.overspec/overspec.yaml` for response language and active team.
2. Read `.overspec/state.json` for current phase and completed artifacts.
3. Read `.overspec/harness/HARNESS.md`.
4. Read `.overspec/harness/feedforward.yaml`.
5. Read `.overspec/harness/sensors.yaml`.
6. Read `.overspec/harness/templates/contract-template.md`.
...
- "Running `bash .overspec/scripts/harness-doctor.sh` reports 0 errors."
...
Use `.overspec/harness/sensors.yaml`.
...
```text
.overspec/harness/contracts/active/{{contract_id}}.md
```
