...
**Descrição:** `overspec.yaml` define 3 perfis de modelo (`quality`, `balanced`, `budget`) com mapeamento agente→modelo. O perfil ativo é `balanced`. Porém, nenhum agente, workflow ou engine doc referencia `model_profiles`. A configuração existe mas é decorativa.

**Evidência:** `overspec.yaml` linhas 57-78 definem os perfis. Nenhuma menção em nenhum arquivo `.agent.yaml`, `workflow-engine.md`, ou `state-machine.md`.
