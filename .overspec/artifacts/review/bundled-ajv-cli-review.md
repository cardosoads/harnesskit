...
Date: 2026-04-13
Contract: `.overspec/harness/contracts/active/HC-20260413-bundled-ajv-cli.md`
...
`ajv-cli@5.0.0` is now a runtime dependency, so an npm install of OverSpec
will install the validator dependency without requiring a separate global
`ajv` install.

`overspec validate` now passes the packaged validator through
`OVERSPEC_AJV_BIN` when the package-local binary exists. Direct script usage
...
- `bash .overspec/scripts/validate.sh` passed: 97 checked, 97 passed.
- `node bin/overspec.mjs validate` passed: 97 checked, 97 passed.
- `bash .overspec/scripts/harness-doctor.sh` passed with 0 errors, 0 warnings,
