...
def apply_baselines(root: Path, issues: list[Issue]) -> None:
    baseline_path = root / ".overspec" / "harness" / "baselines" / "current.yaml"
...
def check_feedforward(root: Path, issues: list[Issue]) -> None:
    overspec = root / ".overspec"
    feedforward_path = overspec / "harness" / "feedforward.yaml"
    ...
    if not required_context:
        required_context = [
            ...
            {"path": ".overspec/overspec.yaml", "min_words": 80, "severity": "error"},
            {"path": ".overspec/state.json", "min_words": 20, "severity": "error"},
            {"path": ".overspec/core/constitution.md", "min_words": 120, "severity": "error"},
            {"path": ".overspec/core/engine/workflow-engine.md", "min_words": 200, "severity": "error"},
            {"path": ".overspec/core/engine/state-machine.md", "min_words": 160, "severity": "error"},
            {"path": ".overspec/harness/HARNESS.md", "min_words": 150, "severity": "error"},
            {"path": ".overspec/harness/templates/contract-template.md", "min_words": 120, "severity": "error"},
        ]
    ...
    config = load_yaml(overspec / "overspec.yaml", issues, "feedforward", "invalid_config") if (overspec / "overspec.yaml").exists() else {}
    state = load_json(overspec / "state.json", issues, "feedforward", "invalid_state") if (overspec / "state.json").exists() else {}

    project_type = state.get("project_type") or config.get("project", {}).get("project_type")
    if not project_type:
        issues.append(Issue("error", "feedforward", "missing_project_type", "No project_type found in state.json or overspec.yaml"))

    active_specs = config.get("specs", {}).get("active", [])
    if not active_specs:
        issues.append(Issue("warning", "feedforward", "missing_active_specs", "No active specs declared in overspec.yaml", "overspec.yaml"))
    for spec in active_specs:
        spec_path = overspec / "specs" / str(spec) / "spec.yaml"
    ...
    if active_team:
        team_path = overspec / "teams" / f"{active_team}.yaml"
        if not team_path.exists():
            add_missing_file(issues, root, "feedforward", team_path)
    else:
        issues.append(Issue("warning", "feedforward", "missing_active_team", "No active team declared in overspec.yaml", "overspec.yaml"))
    ...
    if isinstance(state_phases, dict):
        for phase, pdata in state_phases.items():
            for step in pdata.get("steps", []) if isinstance(pdata, dict) else []:
                ...
                if artifact:
                    artifact_path = overspec / str(artifact)
    ...
    if not workflow_requirements:
        workflow_requirements = [
            {"path": ".overspec/core/workflows/4-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "record-harness-sensors"]},
            {"path": ".overspec/core/workflows/bf-3-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "record-harness-sensors"]},
            {"path": ".overspec/core/workflows/nf-4-implementation/workflow.yaml", "required_steps": ["create-harness-contract", "run-harness-sensors"]},
        ]
    ...
    for contract in (overspec / "harness" / "contracts" / "active").glob("*.md"):
    ...
    if feedforward.get("artifact_quality", {}).get("implementation_report_requires_harness", True):
        implementation_reports = list((overspec / "artifacts" / "implementation").glob("*.md"))
...
def check_feedback(root: Path, issues: list[Issue]) -> None:
    overspec = root / ".overspec"
    sensors_path = overspec / "harness" / "sensors.yaml"
    ...
    if not ({"overharness-validate", "overspec-validate"} & required_ids):
    ...
    evaluations = [p for p in (overspec / "harness" / "evaluations").glob("*.md") if p.name != ".gitkeep"]
    active_contracts = [p for p in (overspec / "harness" / "contracts" / "active").glob("*.md")]
    completed_contracts = [p for p in (overspec / "harness" / "contracts" / "completed").glob("*.md")]
    if active_contracts and not evaluations:
        issues.append(Issue("warning", "feedback", "active_contract_without_evaluation", "Active harness contracts exist but no evaluation artifact has been recorded", rel(root, overspec / "harness" / "evaluations")))
    if completed_contracts and not evaluations:
        issues.append(Issue("warning", "feedback", "completed_contract_without_evaluation", "Completed harness contracts exist but no evaluation artifact has been recorded", rel(root, overspec / "harness" / "evaluations")))
...
def main() -> int:
    ...
    issues: list[Issue] = []
    if not (root / ".overspec").exists():
        issues.append(Issue("error", "feedforward", "missing_overspec_dir", "No .overspec directory found", str(root)))
    else:
