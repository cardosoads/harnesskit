...
def main() -> int:
    ...
    parser.add_argument("--sensor-group", default="implementation", help="Sensor group from .overspec/harness/sensors.yaml")
    parser.add_argument("--output", help="Evaluation output path. Defaults to .overspec/harness/evaluations/<evaluation-id>.md")
    ...
    if not overspec.exists():
        raise SystemExit(f"No .overspec directory found under {root}")

    if args.contract:
        contract_path = resolve_cli_path(root, args.contract).resolve()
    else:
        try:
            contract_path = newest_contract(overspec / "harness" / "contracts" / "active")
    ...
    config = load_yaml(overspec / "overspec.yaml") if (overspec / "overspec.yaml").exists() else {}
    ...
    output_path = resolve_cli_path(root, args.output).resolve() if args.output else overspec / "harness" / "evaluations" / f"{evaluation_id}.md"
